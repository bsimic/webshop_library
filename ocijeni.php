<?php
	...
	//provjera da li je korisnik komentirao knjigu
	$query = "SELECT * FROM ocjena WHERE idKorisnik='" . $idKorisnik . "' AND idKnjiga='" . $idKnjiga . "'";
	$result = mysql_query($query);
	//ako je
	if(mysql_num_rows($result) > 0){
		//update zapisa
		$query = "UPDATE ocjena SET ocjena='" . $ocjena . "' WHERE idKorisnik='" . $idKorisnik . "' AND idKnjiga='" . $idKnjiga . "'";
		$result = mysql_query($query);
		if($result){
			//uspjesan upis
			mysql_close();
		...
		$query = "INSERT INTO ocjena(idKorisnik, idKnjiga, ocjena) VALUES
					('" . $idKorisnik . "','" . $idKnjiga . "','" .  $ocjena . "')";
		$result = mysql_query($query);
